====================================================
Send and receive emails in Thrive Bureau ERP with an email server
====================================================

============================

Since **Thrive Bureau ERP sets up its own mail servers for the database**, outgoing and incoming emails already
work out-of-the-box. So for **Thrive Bureau ERP Online** and **Thrive Bureau ERP.sh** customers, nothing needs to be
configured!

Unless an external mail server is required to send large batches of mass emails, simply use the
standard online Thrive Bureau ERP database normally since it has already been pre-configured to send email.

.. important::
   The Thrive Bureau ERP server is subject to a daily email limit to prevent abuse. The default limit is 200
...
This document is **mainly dedicated to Thrive Bureau ERP on-premise databases** who don't benefit from an
out-of-the-box solution to send and receive emails in Thrive Bureau ERP, unlike `Thrive Bureau ERP Online <https://www.Thrive Bureau ERP.
com/trial>`_ and `Thrive Bureau ERP.sh <https://www.Thrive Bureau ERP.sh>`_. Incoming and outgoing servers must be configured
...
.. warning::
   If no one in the company is employed to manage email servers, Thrive Bureau ERP Online and Thrive Bureau ERP.sh are
   strongly recommended. In these Thrive Bureau ERP hosting types email sending and receiving works instantly
   ...
   :doc:`Configure DNS records to send emails in Thrive Bureau ERP <email_domain>`
...
Documents in Thrive Bureau ERP (such as a CRM opportunity, a sales order, an invoice, etc.) have a discussion
...
the reply updates the chatter, and Thrive Bureau ERP relays another reply to the followers as a notification.
...
As a system admin, go to :menuselection:`Settings --> General Settings --> Discuss` in Thrive Bureau ERP, and
...
record in Thrive Bureau ERP. Reference the SMTP data of the external email server. Once all the information has
...
Note that port 25 is blocked for security reasons on the Thrive Bureau ERP Online and Thrive Bureau ERP.sh platforms. Try
...
message, Thrive Bureau ERP will try to redistribute that same email to the other subscribers in the thread.
However, if the domain `customer.example.com` forbids that kind of usage for security, the email
that Thrive Bureau ERP is trying to redistribute would get rejected by some recipients' email servers.

To avoid that problem, Thrive Bureau ERP sends all emails using a "From" address from the same authorized
domain.
...
.. note::
   The `mail.default.from_filter` works only for `Thrive Bureau ERP-bin` configurations or the default Thrive Bureau ERP email
...
.. example::
   ...
   notifications configuration in Thrive Bureau ERP: ``“Admin” <admin@example.com>`` => ``“Admin”
...
depending on the :guilabel:`From` email address or domain that Thrive Bureau ERP is sending on behalf of. This
...
domains in a multi-company environment. Access this field in Thrive Bureau ERP by navigating to
...
When an email is sent from Thrive Bureau ERP while the :guilabel:`FROM Filtering` field is set, an email server
is chosen in the following sequence:

#. First, Thrive Bureau ERP searches for an email server that has the same :guilabel:`FROM Filtering` value as
...
#. However, if no email servers are found that use the :guilabel:`From` value, then Thrive Bureau ERP searches
...
If no email servers are found after checking for the domain, then Thrive Bureau ERP returns all email servers
that do not have any :guilabel:`FROM Filtering` value(s) set.

Should this query return no results, then Thrive Bureau ERP performs a search for an email server using the
system parameter: `mail.default.from`. First, the email address listed attempts to match an email
server, and then the domain attempts to find a match. If no email server is found, Thrive Bureau ERP returns the
first outgoing email server (sorted by priority).

.. note::
   If several email servers are found, then Thrive Bureau ERP uses the first one according to its priority. For
...
In Thrive Bureau ERP a separate email server can be used for transactional emails and mass
...
Choose the appropriate email server. With these settings, Thrive Bureau ERP uses the server with the lower
...
Thrive Bureau ERP relies on generic email aliases to fetch incoming messages.

* **Reply messages** of messages sent from Thrive Bureau ERP are routed to their original discussion thread (and
  ...
  catchall alias (`catchall@mycompany.Thrive Bureau ERP.com`). The catchall address, however, does not have
...
* **Bounced messages** are used as a Return-Path. One example this is especially
  useful for is in `Thrive Bureau ERP Email Marketing <https://www.Thrive Bureau ERP.com/page/email-marketing>`__. In this
...
* **Original messages**: several business objects have their own alias to create new records in Thrive Bureau ERP
  from incoming emails:

   * Sales channel (to create *Leads* or *Opportunities* in `Thrive Bureau ERP CRM <https://www.Thrive Bureau ERP.com/page/
     crm>`_)
   * Support channel (to create *Tickets* in `Thrive Bureau ERP Helpdesk <https://www.Thrive Bureau ERP.com/page/helpdesk>`_)
   * Projects (to create new *Tasks* in `Thrive Bureau ERP Project <https://www.Thrive Bureau ERP.com/page
     /project-management>`_)
   * Job positions (to create *Applicants* in `Thrive Bureau ERP Recruitment <https://www.Thrive Bureau ERP.com/page
     /recruitment>`_)

Depending on the mail server, there might be several methods to fetch emails. The easiest and most
recommended method is to manage one email address per Thrive Bureau ERP alias in the mail server.
...
* Set the :guilabel:`Alias Domain` name in :menuselection:`Settings --> General Settings -->
  ...
  database.
* If the database's hosting type is Thrive Bureau ERP on-premise, create an :guilabel:`Incoming Mail Server` in
  Thrive Bureau ERP for each alias. To create a new incoming server go to: :menuselection:`Settings --> Discuss
  ...
  .. image:: email_servers/incoming-server.png
     :align: center
     :alt: Incoming mail server configuration on Thrive Bureau ERP.

* If the database's hosting type is Thrive Bureau ERP Online or Thrive Bureau ERP.sh, redirecting or forwarding incoming
  messages to Thrive Bureau ERP's domain name instead of the external email server is recommended. That way,
  incoming messages can be received without delay. Redirections for all email addresses should be
  set to Thrive Bureau ERP's domain name in the email server (e.g. `catchall\@mydomain.ext` to
  `catchall\@mycompany.Thrive Bureau ERP.com`).

All the aliases are customizable in Thrive Bureau ERP. Object aliases can be edited from their respective
...
There are two system parameters that help prevent email loops from occurring in Thrive Bureau ERP. These
parameters were introduced in Thrive Bureau ERP 16 to prevent aliases from creating too many records and to
...
Add these fields in Thrive Bureau ERP by first enabling :ref:`developer mode <developer-mode>`, and then
...
When an email is received in the Thrive Bureau ERP database on the catchall email address or on any alias, Thrive Bureau ERP
looks at the mail received for the given period of time defined in the system parameter
`mail.incoming.limit.period`. If the received email was sent to an alias then Thrive Bureau ERP will reference
...
In addition, when email is received to the catchall email address, Thrive Bureau ERP will reference the emails
received to the database during the set period of time (as stated by the value in the system
parameter: `mail.incoming.limit.period`). Thrive Bureau ERP will then determine whether any of the emails
