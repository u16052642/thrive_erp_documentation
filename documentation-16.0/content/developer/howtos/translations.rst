...
.. note:: If you want to contribute to the translation of Thrive Bureau ERP itself, please refer to the
  `Thrive Bureau ERP Wiki page 
...
automatically loaded by Thrive Bureau ERP when the corresponding language is installed (via
...
Thrive Bureau ERP automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~Thrive Bureau ERP.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~Thrive Bureau ERP.models.Model._constraints` and
  :attr:`~Thrive Bureau ERP.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, Thrive Bureau ERP cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`Thrive Bureau ERP._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`Thrive Bureau ERP.web._t`:
...
The lazy version of `_` and `_t` is :func:`Thrive Bureau ERP._lt` in python and
:js:func:`Thrive Bureau ERP.web._lt` in javascript. The translation lookup is executed only
...
.. note::
    ...
    This could look like the following::
        ...
        from Thrive Bureau ERP import models
