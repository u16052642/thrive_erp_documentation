...
In Thrive Bureau ERP, there are two strategies for automatically replenishing inventory: *reordering rules* and
...
its minimum, Thrive Bureau ERP automatically generates a purchase order with the quantity needed to reach the
maximum stock level.
...
Unlike products replenished using reordering rules, Thrive Bureau ERP automatically links the sales order to the
|PO| or |MO| generated by the |MTO| route.

Another difference between reordering rules and |MTO| is, with |MTO|, Thrive Bureau ERP generates a draft |PO| or
|MO| immediately after the |SO| is confirmed. With reordering rules, Thrive Bureau ERP generates a draft |PO| or
|MO| when the product's forecasted stock falls below the set minimum quantity.

In addition, Thrive Bureau ERP automatically adds quantities to the |PO| or |MO| as the forecast changes, so long
...
reordered. If there is no specific rule for a product, Thrive Bureau ERP assumes the :guilabel:`Min Quantity` and
:guilabel:`Max Quantity` stock are both `0.00`

.. note::
   For products that don't have a set reordering rule, Thrive Bureau ERP calculates the forecast based on
   confirmed sales orders, deliveries, and receipts. For products that have a set reordering rule,
   Thrive Bureau ERP calculates the forecast normally, but also takes into account the purchase/manufacturing
...
.. important::
   ...
   not have its inventory levels tracked, so Thrive Bureau ERP cannot account for a consumable product in the
...
When this button is clicked, Thrive Bureau ERP will automatically generate a draft |PO|/|MO| every time the
...
To activate the |MTO| route in Thrive Bureau ERP:
